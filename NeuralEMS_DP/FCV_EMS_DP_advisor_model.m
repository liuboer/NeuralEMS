...
if par.gpu_bool
veh_spd = gpuArray(veh_spd);
veh_acc = gpuArray(veh_acc);
fcs_pwr = gpuArray(fcs_pwr);
ess_soc = gpuArray(ess_soc);
end
%% parametas
% Vehicle parametas
...
mc_trq = T_whl .* eff_fd.^(-sign(T_whl)) ./ veh_fd_ratio;
...
mc_inpwr =  mc_outpwr .* ((mc_eff.*eff_dc_ac).^(-sign(mc_outpwr)));

% Fuel cell
fc_fuel = interp1(fc_pwr_map, fc_fuel_map2, fcs_pwr, 'linear', 'extrap') .* par.dt;
inf_fc = (fcs_pwr < fc_pwr_min) + (fcs_pwr > fc_pwr_max);

% Battery
ess_pwr = mc_inpwr - fcs_pwr .* eff_dc_dc;
...
ess_soc_new = ess_soc - ess_cur .* par.dt ./ ess_Q;
