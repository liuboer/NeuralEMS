function varargout  = dpm_gpu(varargin)
...
try
    ...
    for i=1:length(grd.Nx)
        out.I = bitor(out.I,uint32(out.X{i}>grd.Xn{i}.hi(n+1)));
        out.I = bitor(out.I,uint32(out.X{i}<grd.Xn{i}.lo(n+1)));
    end
    J  = double(out.I==0).*out.C{1} + double(out.I).* options.MyInf;
...
optb.Warnings     = 'off';
dynb              = dpm_gpu(@dpm_model_inv,parb,grdb,dis,optb);
...
for n=dis.N0:dis.N
    ...
    if ~options.UseUmap
        ...
        % take care of bounds
        for i=1:length(grd.Nx)
            I = bitor(I,uint32(X{i}>grd.Xn{i}.hi(n+1)));
            I = bitor(I,uint32(X{i}<grd.Xn{i}.lo(n+1)));
        end
        % if line was caluclated take care of bounds again
        if options.UseLine
            I = bitor(I,uint32(X{i}>dyn.B.hi.Xo(n+1)));
            I = bitor(I,uint32(X{i}<dyn.B.lo.Xo(n+1)));
        end
